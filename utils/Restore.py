...
def restore(args, model, best_weights_path):

    group = args.group
    savedir = os.path.join(args.snapshot_dir, args.arch, best_weights_path, 'group_%d_of_%d'%(group, args.num_folds), 'ckpt')
    filename= 'best_weights.pth.tar' #'step_%d.pth.tar'%(args.restore_step)
...
def get_save_dir(args):
    ...
    snapshot_dir = os.path.join(args.snapshot_dir, args.arch, d, 'group_%d_of_%d'%(args.group, args.num_folds))
    return snapshot_dir
