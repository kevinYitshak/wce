...
def check(args):
    ...
    for data in val_dataloader:
        ...
        query_img, query_mask, support_img, support_mask, class_, size = data
        print(query_img.size(), query_mask.size(), support_img.size(), support_mask.size(), class_, size)
    ...
    for group in range(4):
        ...
        for data in val_dataloader:
            ...
            query_img, query_mask, support_img, support_mask, idx \
                = query_img.to(device), query_mask.to(device), support_img.to(device), support_mask.to(device), idx.to(device)
