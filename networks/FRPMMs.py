...
class OneModel(nn.Module):
    ...
    def forward(self, query_rgb, support_rgb, support_mask):
        ...
        for num in self.num_pro_list:
            self.num_pro = num
            self.PMMs = PMMs(256, k=num).to(self.device) #cuda()
            ...
            # print('exit_feat_in: ', exit_feat_in.shape)
            # segmentation
            out, out_sigmoid = self.Segmentation(exit_feat_in, Pseudo_mask)
            # print('out_sigmoid: ', out_sigmoid.shape)
            Pseudo_mask = out_sigmoid
            out_list.append(out)
    ...
    def Segmentation(self, feature, history_mask):
        ...
        out = feature
        # print('feture_size: {0}; history_mask_size: {1}'.format(feature.size(), history_mask.size()))
        out_plus_history = torch.cat([feature, history_mask], dim=1)
        # print('one_plus_history: ', out_plus_history.shape)
