...
class OneModel(nn.Module):
    def __init__(self, args):
        ...
        self.PMMs = PMMs(256, self.num_pro).to(self.device) #PMMs(256, self.num_pro).cuda()
    ...
    def get_loss(self, logits, query_label, idx):
        ...
        print('size query label: ', query_label.size())
        bb, cc, _, _ = outB_side.size()
        outB_side = outB_side.view(b, -1)
        print('batch size query pred: ', outB_side.size())
        #

        loss_bce_seg = bce_logits_func(outB_side, query_label) # query_label.long())
    ...
    def get_pred(self, logits, query_image):
        ...
        # out_sigmoid = F.sigmoid(outB_side)
        return outB_side # out_softmax, pred
